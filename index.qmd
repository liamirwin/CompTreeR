...
In this document we will provide a brief overview of the processing steps required to go from a raw point cloud to a individual tree competition metric layer. This workflow is based on the methods outlined in [Irwin et al. 2024](http://doi.org/10.1093/forestry/cpae030) and is designed to be used with the R programming language.
...
First we will detect and segment tree crowns, there are many methods to achieve this and they should be selected based on the objectives, forest type, and avaliable compute power [(more details: lidRbook/itd-its)](https://r-lidar.github.io/lidRbook/itd-its.html). In this study we applied a modified marker-based watershed approach. Thanks to the distinct conical shape and distribution of the trees in these conifer dominated stands, this approach worked well for detecting and segmenting dominant and co-dominant trees that are the target for commercial thinning.

We will start by loading relevant packages and an example point cloud from the [DJI L1](https://enterprise.dji.com/zenmuse-l1) lidar dataset; a subset of the drone laser scanning data clipped from the [Irwin et al. 2024](http://doi.org/10.1093/forestry/cpae030) coverage. See the manuscript for details on acquisition parameters.
...
Download the point cloud [here (8.05 mb)](extdata/thin_plot_dec.laz)
